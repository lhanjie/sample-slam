...
// 给定index，读取一帧数据
vector<string> rgb_path, depth_path;
void init(string txtFile) 
{
    ifstream fAssociation;
    fAssociation.open(txtFile.c_str());
    while(!fAssociation.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            ss >> t;
            ss >> sRGB;
            rgb_path.push_back(sRGB);
            ss >> t;
            ss >> sD;
            depth_path.push_back(sD);
        }
    }
}
...
int main( int argc, char** argv )
{
    ...
    string txtFile = pd.getData("associate_txt");
    init(txtFile);
    // 所有的关键帧都放在了这里
    ...
    *******************************/
    ...
    for (size_t i=0; i<keyframes.size(); i++)
    {
        ...
        *output += *tmp;
        ...
        //查看拼接过程（测试）
        //cloud = joinPointCloud( cloud, currFrame, pose.matrix(), camera );
        if ( visualize == true )
...
FRAME readFrame( int index, ParameterReader& pd )
{
    ...
    string Dir = pd.getData("dir");
    f.rgb = cv::imread(Dir+rgb_path[index]);
    f.depth = cv::imread(Dir+depth_path[index], -1);
